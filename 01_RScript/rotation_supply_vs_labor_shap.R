...
################################ GRAPH
# Author: Ece Tasan (original) — Economist retheme by Emilia
# Date  : 5/26/2026 (updated)
# Scope : Same as original — SHAP rotation charts (3-month horizon)
#         Changes from original:
#           - Economist-style theme (serif fonts, top rule, clean grid)
#           - New color palette: #6161BA supply, #34BA66 labour
#           - Ribbon fills updated to match new colors
#           - plot_annotation titles/subtitles rethemed
...
# ============================================================
# Palette
#6161BA  — periwinkle blue   -> supply
# #34BA66  — mid green         -> labour
# #BAA237  — warm gold         -> accent / rule
# #BA3B35  — deep red          -> available for extra series
# #555566  — dark slate        -> text / grid
# ============================================================

COL_SUPPLY <- "#6161BA"
COL_LABOUR <- "#34BA66"
COL_RULE   <- "#6161BA"   # top border accent (matches supply)
COL_TEXT   <- "#555566"   # axis text, captions
...
theme_ec <- function(base_size = 10) {
...
# ============================================================
# Load saved SHAP runs (unchanged)
# ============================================================
...
setDT(fred)
oos_dates_1 <- tail(fred$date[fred$date < as.Date("2016-01-01")],
                    length(shap1$save.shap))
oos_dates_2 <- tail(fred$date, length(shap2$save.shap))
...
# ============================================================
# Per-window aggregations (unchanged)
# ============================================================
...
# ============================================================
# 12-month rolling means (unchanged)
# ============================================================
...
build_rel_panel <- function(d, yvar, title, line_color, show_x = TRUE) {
  ...
  p <- ggplot(d2, aes(x = date, y = .data[[yvar]])) +
    annotate("rect", xmin = gfc_start, xmax = gfc_end,
             ymin = 0, ymax = ymax, fill = "#CCCCDD", alpha = 0.50) +
    annotate("rect", xmin = cov_start, xmax = cov_end,
             ymin = 0, ymax = ymax, fill = "#CCCCDD", alpha = 0.50) +
    geom_ribbon(aes(ymin = 1, ymax = pmax(.data[[yvar]], 1)),
                fill = line_color, alpha = 0.18) +
    geom_ribbon(aes(ymin = pmin(.data[[yvar]], 1), ymax = 1),
                fill = line_color, alpha = 0.07) +
    geom_hline(yintercept = 1, linetype = "22",
               color = "#AAAABC", linewidth = 0.45) +
...
build_signed_panel <- function(d, yvar, title, line_color, show_x = TRUE) {
  ...
  p <- ggplot(d2, aes(x = date, y = .data[[yvar]])) +
    annotate("rect", xmin = gfc_start, xmax = gfc_end,
             ymin = ymin, ymax = ymax, fill = "#CCCCDD", alpha = 0.50) +
    annotate("rect", xmin = cov_start, xmax = cov_end,
             ymin = ymin, ymax = ymax, fill = "#CCCCDD", alpha = 0.50) +
    geom_ribbon(aes(ymin = 0, ymax = pmax(.data[[yvar]], 0)),
                fill = line_color, alpha = 0.18) +
    geom_ribbon(aes(ymin = pmin(.data[[yvar]], 0), ymax = 0),
                fill = line_color, alpha = 0.07) +
    geom_hline(yintercept = 0, color = COL_TEXT, linewidth = 0.45) +
    geom_line(color = line_color, linewidth = 0.95) +
    scale_x_date(date_breaks = "2 years", date_labels = "%Y",
                 limits = x_lims,
                 expand = expansion(mult = c(0.01, 0.02))) +
...
p1_top <- build_rel_panel(dt, "supply_rel", SUPPLY_TITLE,
                          line_color = COL_SUPPLY, show_x = FALSE)
p1_bot <- build_rel_panel(dt, "labor_rel",  LABOR_TITLE,
                          line_color = COL_LABOUR, show_x = TRUE)
...
# ============================================================
# Save (unchanged paths and dimensions)
# ============================================================
...
ggsave(out1, plot = chart1, width = 14, height = 8, dpi = 300, bg = "white")
ggsave(out2, plot = chart2, width = 14, height = 8, dpi = 300, bg = "white")
