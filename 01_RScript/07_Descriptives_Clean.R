...
COL_INFL   <- "#6161BA"   # periwinkle  — inflation / main series
COL_VU     <- "#34BA66"   # green       — v/u ratio
...
# ---- 4. Rolling 36-month Phillips correlation (unchanged) ----------------
setorder(DT, date)
DT[, roll_corr := zoo::rollapplyr(
  data      = cbind(infl_mom, UNRATE),
  width     = 36,
...
# Shock bands — shared across all three panels
shock_bands <- list(
  geom_rect(data = shocks,
...
p1b <- ggplot(DT, aes(x = date, y = vu_ratio)) +
  ...
  x_scale +
  labs(title = "(b) Vacancy / Unemployment ratio", y = "ratio") +
  theme_ec()
...
p1c <- ggplot(DT, aes(x = date, y = roll_corr)) +
  ...
  x_scale +
  labs(title = expression(bold("(c) 36-month rolling correlation ("*pi*", u)")),
       y = expression(rho)) +
  theme_ec()
...
fig1 <- (p1a / p1b / p1c) +
  plot_annotation(
    ...
    subtitle = "Monthly CPI inflation, vacancy-unemployment ratio, and rolling Phillips correlation",
...
# ---- 6. Figure 2: Phillips scatter by regime -----------------------------
...
p2 <- ggplot(DT, aes(x = UNRATE, y = infl_mom, colour = regime2)) +
  ...
  scale_colour_manual(
    ...
    guide = guide_legend(
      override.aes = list(size = 3, alpha = 0.85, linetype = 0)
    )
  ) +
  labs(
    title    = "The Phillips curve by regime",
    subtitle = "Monthly CPI inflation vs. unemployment rate, 2002–2024",
    x        = "Unemployment rate, level (%)",
    y        = "Monthly CPI inflation (%)",
