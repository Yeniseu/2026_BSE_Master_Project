...
# ============================================================
# Palette + label sizing
# ============================================================

COL_SUPPLY     <- "#6161BA"   # 1st palette colour -> supply
COL_LABOR      <- "#34BA66"   # 2nd palette colour -> labor
SIZE_SHOCK_LBL <- 3.5         # GFC / COVID labels
...
build_rel_panel <- function(d, yvar, title, line_color, show_x = TRUE,
  ...
  top_exp <- if (label_shocks) 0.08 else 0      # headroom for GFC/COVID labels
  ...
  p <- ggplot(d2, aes(x = date, y = .data[[yvar]])) +
    ...
    scale_y_continuous(limits = c(0, ymax),
                       ...
                       expand = expansion(mult = c(0, top_exp))) +
    labs(title = title, x = NULL,
         y = "Relative |SHAP|\n(1.0 = own average)") +
    theme_minimal(base_size = 10, base_family = PLOT_FONT) +
    theme(
      plot.title         = element_text(face = "bold", size = 12),
      axis.title.y       = element_text(size = 13, colour = "grey25"),
  ...
  if (label_shocks) {
    p <- p +
      annotate("text", x = gfc_mid, y = Inf, label = "GFC",
               vjust = 1.3, fontface = "bold", family = PLOT_FONT,
...
# ============================================================
# Shared plot_annotation theme (Computer Modern)
# ============================================================

annotation_theme <- theme(
  plot.title    = element_text(face = "bold", size = 12, family = PLOT_FONT),
  plot.subtitle = element_text(colour = "grey25", size = 10, family = PLOT_FONT),
...
# ============================================================
# show_x = TRUE on both panels (timeline on upper panel too)
# label_shocks = TRUE on both panels (GFC/COVID on lower panel too)

p1_top <- build_rel_panel(dt, "supply_rel", SUPPLY_TITLE,
                          line_color = COL_SUPPLY, show_x = TRUE,
                          label_shocks = TRUE)
p1_bot <- build_rel_panel(dt, "labor_rel",  LABOR_TITLE,
                          line_color = COL_LABOR,  show_x = TRUE,
                          label_shocks = TRUE)
...
ggsave(out1, plot = chart1, width = 10, height = 6, dpi = 300)
ggsave(out2, plot = chart2, width = 10, height = 6, dpi = 300)
